...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        if is_codex_backend:
            # The chatgpt.com Codex backend needs a stable request/session
            # header for prompt-cache routing.  A prior change removed these
            # headers after mistaking ``extra_headers`` for a body field; the
            # Responses SDK treats it as HTTP request headers, and without the
            # stable ``session_id`` / ``x-client-request-id`` pair GPT-5.5
            # resumes collapse from ~98% cache hits to effectively uncached.
